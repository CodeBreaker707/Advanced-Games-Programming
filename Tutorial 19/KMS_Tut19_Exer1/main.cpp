...
HRESULT InitialiseGraphics()// 03-01
{
	...
	sky = new SkyBox();
	sky->InitialiseSkyBox(g_pD3DDevice, g_pImmediateContext, "Assets/cubeObj.obj", "Assets/skybox02.dds");
	...
	g_node2 = new SceneNode(4.0f, 0.0, 0.0f);
	g_node3 = new SceneNode(20.0f, 0.0, 15.0f);

	g_node1->SetModel(sphere);
	g_node2->SetModel(p_sphere);
	g_node3->SetModel(p_sphere);

	g_root_node->AddChildNode(g_node1);
	g_node1->AddChildNode(g_node2);
	g_root_node->AddChildNode(g_node3);
	//g_node1->AddChildNode(g_node2);
...
void RenderFrame(void)
{
	...
	g_node1->IncX(0.001, g_root_node);
	//g_node3->IncX(-0.001, g_root_node);

	//p_sphere->Draw(&view, &projection);
	//sphere->Draw(&view, &projection);

	sky->DrawSkyBox(&view, &projection, eye->GetPosition(), 1.5f);
