...
char		g_Tutorial_04_Advanced_01[100] = "Tutorial 04 Advanced 01\0";
...
HRESULT InitialiseWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hWnd = CreateWindow(Manikandan, g_Tutorial_04_Advanced_01, WS_OVERLAPPEDWINDOW,
...
HRESULT InitialiseGraphics()// 03-01
{
	...
	//Creating a second constant buffer
	D3D11_BUFFER_DESC constant_buffer_descr;
	ZeroMemory(&constant_buffer_descr, sizeof(constant_buffer_descr));

	constant_buffer_descr.Usage = D3D11_USAGE_DEFAULT; // Can use UpdateSubresource() to update
	constant_buffer_descr.ByteWidth = 16; // MUST be a multiple of 16, calculate from CB struct
	constant_buffer_descr.BindFlags = D3D11_BIND_CONSTANT_BUFFER; // Use as a constant buffer

	hr = g_pD3DDevice->CreateBuffer(&constant_buffer_descr, NULL, &g_pConstantBuffer1);
...
void RenderFrame(void)
{
	...
	UINT offset = 0;
	g_pImmediateContext->IASetVertexBuffers(1, 1, &g_pVertexBuffer, &stride, &offset);

	CONSTANT_BUFFER0 cb0_values;
	cb0_values.ColorAmount = 0.5f; // 47% of vertex red value

	CONSTANT_BUFFER1 cb1_values;
	cb1_values.ScaleAmount = 0.5f;
	...
	g_pImmediateContext->VSSetConstantBuffers(1, 1, &g_pConstantBuffer1);
