...
struct CONSTANT_BUFFER0
{
	...
	XMVECTOR point_light_position; // 16 bytes
	XMVECTOR point_light_colour; // 16 bytes
...
void RenderFrame(void)
{
	...
	g_point_light_position = XMVectorSet(0.0f, 0.0f, -5.0, 0.0f);
	g_point_light_colour = XMVectorSet(1.0f, 1.0f, 1.0f, 0.0f);
	...
	inverse = XMMatrixInverse(&determinant, world2);
	//light_rot_matrix = XMMatrixRotationRollPitchYaw(lx_rotation, 0.0f, 0.0f);

	cb0_values.point_light_colour = g_point_light_colour;
	cb0_values.ambient_light_colour = g_ambient_light_colour;

	cb0_values.point_light_position = XMVector3Transform(g_point_light_position, inverse);
